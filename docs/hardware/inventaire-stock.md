...
| **Petits OLED I²C** — *trou déclaré puis COMBLÉ le 14/09, en 25 min* | Marquages lus au dos : `IIC ADRESS SELECT` avec pont **`0x78` / `0x7A`** (⇒ **0x3C / 0x3D** en adressage 7 bits). Recto : `GND · VCC · SCL · SDA`. ⚠️ **Contrôleur NON lisible** (sous la dalle) : `SSD1306` et `SH1106` sont indiscernables à l'œil, et le SH1106 décale l'image de 2 px — ça se tranche au premier affichage, pas sur photo. ⚠️ Sur l'exemplaire photographié, la dalle **paraît soulevée d'un côté** : à vérifier à la main | module carré à 4 trous, dalle collée, barrette 4 broches | **« plein »** selon David — **toujours non compté** | `photos/2026-09-14-module-joystick/oled-*` | **H** (brochage, adresse) / ⛔ (contrôleur, qté) |
