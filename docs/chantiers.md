...
| **Télécommande USB RP2040** | ÉTUDE ÉCRITE le 14/09 — [`etude-telecommande-usb.md`](etude-telecommande-usb.md), rien de fabriqué. Née de 4 photos d'atelier d'un module joystick **`HW-504`** (réf lue ; ⚠️ `R5` NON peuplée ⇒ `SW` flottant ⇒ pull-up interne obligatoire ; ⚠️ la broche marquée `+5V` doit être alimentée en **3,3 V**, sinon les ADC du RP2040 prennent 5 V pour un max absolu de 3,63 V). Destination **double** : télécommande Pi 4 (série USB CDC, trame maison) **et** simulation (manette HID standard lue par `joy_node`/`teleop_twist_joy` — zéro code ROS à écrire). Carte retenue : **XIAO RP2040** (USB-C, 4 ADC, en stock) — mais **11 broches seulement**, le décompte passe sans marge. ⭐ **Le seul chantier qui avance sans le robot** (banc + sim), donc utile pendant le gel du Pi 4 | **T0** : vérifier sur matériel que CircuitPython tient **HID + CDC-data ensemble** (aucun exemple officiel trouvé, endpoints RP2040 non chiffrés nulle part) — 1 soirée, et ça peut invalider l'architecture à lui seul. **Rien à souder ni imprimer avant sa réponse.** Puis T1 protocole testé sur l'hôte | **aucun** pour T0-T3 (banc + sim, robot pas concerné). ⚠️ Le jour où il commande des roues RÉELLES il entre dans le chemin roues ⇒ sim + protocole caméra + revue Opus. **4 arbitrages en attente de David** : rôle des 4 boutons (4 actions directes — recommandé — ou 4 touches de menu), `SW` câblé ou non (il coûte la dernière voie libre), référence de l'OLED, nb exact de XIAO en stock |
