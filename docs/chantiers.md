...
| Odométrie des roues (encodeurs) | **disque IMPRIMÉ et monté le 19/08** (plaqué contre la couronne, rondelles — vigilances fluage/faux-rond au README plans) ; support capteurs : **v4 MORTE AU MONTAGE 19/08** (le palier occupe le volume), **direction v5 = pince sur la vis de suspension du palier** (double écrou, hors chemin d'effort) ; capteurs LJ12A3 reçus | **mesurer le bloc E** de `plans/odometrie/MESURES.md` (filetage libre, déports, anti-rotation) → dessiner la patte v5 + vue de montage à faire valider AVANT impression | cotes E toutes à prendre ; restent aussi `ROUE_D` (hyp. 250) et l'entraxe roues (C2) |
